...
%   - 右列：统计结果与可视化
    ...
    handles.hPeakRatio = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
        'Units', 'normalized', ...
        'Position', [0.60 0.83 0.28 0.10], ...
        'String', '1.15');

    % Z边界层数
    uicontrol('Parent', paramPanel, 'Style', 'text', ...
        'Units', 'normalized', ...
        'Position', [0.05 0.67 0.52 0.10], ...
        'String', 'Z边界层数:', ...
        'ForegroundColor', textColor, ...
        'BackgroundColor', [0.07 0.08 0.11], ...
        'FontName', 'Times New Roman', ...
    ...
    handles.hEdgeMargin = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
        'Units', 'normalized', ...
        'Position', [0.60 0.67 0.28 0.10], ...
        'String', '2');

    % 二维识别阈值
    uicontrol('Parent', paramPanel, 'Style', 'text', ...
        'Units', 'normalized', ...
        'Position', [0.05 0.50 0.52 0.10], ...
        'String', '二维识别阈值', ...
        'ForegroundColor', textColor, ...
        'BackgroundColor', [0.07 0.08 0.11], ...
        'FontName', 'Times New Roman', ...
    ...
    handles.hROIScale = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
        'Units', 'normalized', ...
        'Position', [0.60 0.33 0.28 0.10], ...
        'String', '0.50');

    % ROI最小半径
    uicontrol('Parent', paramPanel, 'Style', 'text', ...
        'Units', 'normalized', ...
        'Position', [0.05 0.17 0.52 0.10], ...
        'String', 'ROI最小半径', ...
        'ForegroundColor', textColor, ...
        'BackgroundColor', [0.07 0.08 0.11], ...
        'FontName', 'Times New Roman', ...
    ...
    handles.hROIMinRadius = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
        'Units', 'normalized', ...
        'Position', [0.60 0.17 0.28 0.10], ...
        'String', '15');

    % 最小有效粒径 (um) — 低于此值不画框
    uicontrol('Parent', paramPanel, 'Style', 'text', ...
        'Units', 'normalized', ...
        'Position', [0.05 0.00 0.52 0.10], ...
        'String', '最小有效粒径(um)', ...
        'ForegroundColor', [1.0 0.85 0.3], ...
        'BackgroundColor', [0.07 0.08 0.11], ...
        'FontName', 'Times New Roman', ...
        'FontSize', 9, ...
        'HorizontalAlignment', 'left');
    handles.hMinValidDiamUm = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
        'Units', 'normalized', ...
        'Position', [0.60 0.00 0.28 0.10], ...
        'String', '0', ...
        'TooltipString', '低于此粒径(um)的粒子不画框, 0=不过滤');
    ...
    function onReconstruct(~, ~)
        ...
        minValidDiamUm = str2double(get(handles.hMinValidDiamUm, 'String'));
        ...
        if isnan(minValidDiamUm), minValidDiamUm = 0; end
        ...
        handles.minValidDiamUm = minValidDiamUm;
        handles.simulationMode = simulationMode;
...
% =========================================================================
    ...
    function iUpdateStatsDisplay(handlesIn)
        ...
        summaryLines = { ...
            ...
            sprintf('粒径下限: %.2f %cm', statsResult.minValidDiamUm, muChar), ...
        ...
        % 更新表格
        shapeType = statsResult.shapeType(:);
    ...
    function [handlesOut, statsResult, errMsg] = iBuildStatsResult(handlesIn)
        ...
        if isfield(fastStats, 'candidateCoords3D') && ~isempty(fastStats.candidateCoords3D)
            ...
            candidateCoordsPx = fastStats.coords3D;
        end
        if isfield(fastStats, 'candidateRoiBoxes') && ~isempty(fastStats.candidateRoiBoxes)
            roiBoxesAll = fastStats.candidateRoiBoxes;
        else
            roiBoxesAll = fastStats.roiBoxes;
        end
        if isfield(fastStats, 'candidateStats') && ~isempty(fastStats.candidateStats)
            candidateStats = fastStats.candidateStats;
        else
            candidateStats = fastStats.stats;
        end

        nCand = size(candidateCoordsPx, 1);
        if isfield(fastStats, 'validMask') && ~isempty(fastStats.validMask)
            validMaskAll = logical(fastStats.validMask(:));
        else
            validMaskAll = true(nCand, 1);
        end
        if numel(validMaskAll) < nCand
            validMaskAll(end + 1:nCand, 1) = false;
        else
            validMaskAll = validMaskAll(1:nCand);
        end

        minValidDiamUm = 0;
        if isfield(handlesIn, 'minValidDiamUm') && ~isempty(handlesIn.minValidDiamUm)
            minValidDiamUm = max(0, double(handlesIn.minValidDiamUm));
        end
        ...
        for k = 1:nCand
            if k <= numel(candidateStats)
                s = candidateStats(k);
                areaPx = 0;
                if isfield(s, 'Area') && isfinite(s.Area) && s.Area > 0
                    areaPx = double(s.Area);
                    candDiamUm(k) = 2 * sqrt(areaPx / pi) * pix_um;
                elseif isfield(s, 'EquivDiameter') && isfinite(s.EquivDiameter) && s.EquivDiameter > 0
                    candDiamUm(k) = double(s.EquivDiameter) * pix_um;
                end
                if areaPx > 0 && isfield(s, 'Perimeter') && isfinite(s.Perimeter) && s.Perimeter > 0
                    candCircularity(k) = 4 * pi * areaPx / max(double(s.Perimeter) ^ 2, eps);
                end
            end
        ...
        if isempty(validIdx)
            errMsg = '当前粒径下限下没有有效粒子，请降低最小有效粒径或检查阈值。';
            return;
        end

        coordsPx = candidateCoordsPx(validIdx, :);
        diamsUm = candDiamUm(validIdx);
    ...
    function hOverview = iShowParticleOverview(statsResult)
        ...
        if numel(validMask) < size(roiBoxes, 1)
            validMask(end + 1:size(roiBoxes, 1), 1) = false;
        else
            validMask = validMask(1:size(roiBoxes, 1));
        end

        candShapeType = repmat({'irregular'}, size(roiBoxes, 1), 1);
        if isfield(statsResult, 'candidateShapeType') && ~isempty(statsResult.candidateShapeType)
            srcShapeType = statsResult.candidateShapeType;
            if isstring(srcShapeType)
                srcShapeType = cellstr(srcShapeType);
            end
            nShape = min(numel(srcShapeType), numel(candShapeType));
            candShapeType(1:nShape) = srcShapeType(1:nShape);
        elseif isfield(statsResult, 'candidateCircularity') && ~isempty(statsResult.candidateCircularity)
            circThreshold = 0.75;
            if isfield(statsResult, 'shapeCircularityThreshold')
                circThreshold = statsResult.shapeCircularityThreshold;
            end
            for nb = 1:min(numel(statsResult.candidateCircularity), numel(candShapeType))
                if statsResult.candidateCircularity(nb) >= circThreshold
                    candShapeType{nb} = 'circular';
                end
            end
        end

        minD = 0;
        if isfield(statsResult, 'minValidDiamUm')
            minD = max(0, statsResult.minValidDiamUm);
        end
        ...
        for n = 1:size(roiBoxes, 1)
            ...
            isCirc = strcmpi(candShapeType{n}, 'circular');
            if isCirc
                boxColor = [0.20 0.95 0.55];  % green: circular
                nCirc = nCirc + 1;
            else
                boxColor = [1.00 0.65 0.20];  % orange: irregular
                nIrreg = nIrreg + 1;
            end
        ...
        statusStr = sprintf('候选:%d  有效显示:%d  圆(绿):%d  不规则(橙):%d  粒径下限:%.0fum', ...
            candidateCount, shownCount, nCirc, nIrreg, minD);
        text(axMip, 0.02, 0.98, statusStr, ...
    ...
    function iExportStatsResult(statsResult, targetPath, filterIdx)
        ...
        particleTbl = table((1:statsResult.count).', statsResult.coordsUm(:, 1), statsResult.coordsUm(:, 2), ...
            statsResult.coordsUm(:, 3), statsResult.diamsUm(:), statsResult.volumeUm3(:), shapeType, ...
            'VariableNames', {'ID', 'x_um', 'y_um', 'z_um', 'Heywood_d_um', 'V_single_um3', 'shape_type'});
        % 构建摘要表格
        summaryRows = { ...
            ...
            'min_valid_diameter_um', statsResult.minValidDiamUm; ...
