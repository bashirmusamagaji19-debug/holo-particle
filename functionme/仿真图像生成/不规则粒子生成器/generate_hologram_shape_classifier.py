...
def _generate_aggregate_sub_spheres(
    ...
    """Generate randomly placed overlapping sub-spheres within a bounding sphere.

    Returns (centers_um, radii_um) where centers are 3D coordinates relative to
    the aggregate centroid.
    """
    centers = np.zeros((num_sub_spheres, 3))
    radii = np.zeros(num_sub_spheres)

    # Seed sphere at origin
    radii[0] = rng.uniform(0.25, 0.45) * target_radius_um
    centers[0] = [0.0, 0.0, 0.0]

    for i in range(1, num_sub_spheres):
        placed = False
        for _ in range(100):  # max attempts per sphere
            # Random point within bounding sphere, biased toward outer region
            phi = rng.uniform(0, 2 * np.pi)
            cos_theta = rng.uniform(-1, 1)
            theta = np.arccos(cos_theta)
            r = target_radius_um * rng.uniform(0.0, 1.0) ** (1.0 / 3.0)

            cx = r * np.sin(theta) * np.cos(phi)
            cy = r * np.sin(theta) * np.sin(phi)
            cz = r * np.cos(theta)

            sub_r = rng.uniform(0.12, 0.35) * target_radius_um

            # Check overlap with at least one existing sphere (connected cluster)
            dists = np.sqrt(
                (centers[:i, 0] - cx) ** 2
                + (centers[:i, 1] - cy) ** 2
                + (centers[:i, 2] - cz) ** 2
            )
            # Allow some overlap so cluster is connected
            if np.any(dists < (radii[:i] + sub_r) * 0.8):
                centers[i] = [cx, cy, cz]
                radii[i] = sub_r
                placed = True
                break
        ...
        if not placed:
            # Fallback: place near a random existing sphere
            parent = rng.integers(0, i)
            phi = rng.uniform(0, 2 * np.pi)
            cos_theta = rng.uniform(-1, 1)
            theta = np.arccos(cos_theta)
            sub_r = rng.uniform(0.12, 0.35) * target_radius_um
            overlap = rng.uniform(0.3, 0.7) * (radii[parent] + sub_r)
            direction = np.array(
                [np.sin(theta) * np.cos(phi), np.sin(theta) * np.sin(phi), np.cos(theta)]
            )
            centers[i] = centers[parent] + direction * overlap
            radii[i] = sub_r
...
def _voxelize_sphere_cluster(
    ...
    """Voxelize a cluster of spheres into a 3D binary grid.

    Returns:
        thickness_map_um: 2D array of shape (ny, nx) — thickness along z at each (x,y)
        volume_um3: float — total occupied volume
        x_edges, y_edges: voxel grid edges in um (for coordinate mapping)
    """
    ...
    X, Y, Z = np.meshgrid(voxel_centers, voxel_centers, voxel_centers, indexing="ij")

    occupied = np.zeros((n_voxels, n_voxels, n_voxels), dtype=bool)

    for c, r in zip(centers_um, radii_um):
        dist_sq = (X - c[0]) ** 2 + (Y - c[1]) ** 2 + (Z - c[2]) ** 2
        occupied |= dist_sq < r**2

    # Project along z (axis 2) to get thickness
    thickness_map_um = np.sum(occupied, axis=2, dtype=np.float64) * voxel_um
    volume_um3 = np.sum(occupied) * voxel_um**3
...
class PolyhedronShape:
    ...
    def initialize(self, rng: np.random.Generator, voxel_um: float = 0.5):
        # Generate random points on a sphere surface, then scale
        points = rng.normal(0, 1, (self.num_vertices * 3, 3))
        norms = np.linalg.norm(points, axis=1, keepdims=True)
        points = points / norms * self.target_radius_um

        # Apply aspect ratio stretching
        points[:, 0] *= self.aspect_ratio[0]
        points[:, 1] *= self.aspect_ratio[1]
        points[:, 2] *= self.aspect_ratio[2]

        # Add some interior points to make hull more "blocky"
        n_interior = self.num_vertices // 2
        if n_interior > 0:
            interior = rng.uniform(-0.6, 0.6, (n_interior, 3)) * self.target_radius_um
            interior[:, 0] *= self.aspect_ratio[0]
            interior[:, 1] *= self.aspect_ratio[1]
            interior[:, 2] *= self.aspect_ratio[2]
...
def _point_in_convex_hull(points: np.ndarray, hull_vertices: np.ndarray) -> np.ndarray:
    """Test if points are inside a convex hull using half-space representation.

    Args:
        points: (N, 3) array of query points
        hull_vertices: (M, 3) vertices of the convex hull

    Returns:
        (N,) boolean array
    """
    hull = ConvexHull(hull_vertices)
    eq = hull.equations  # (F, 4) — each row: A*x + B*y + C*z + D <= 0 for interior
    # Test all points against all faces
    inside = np.ones(len(points), dtype=bool)
    for face_eq in eq:
        inside &= (points @ face_eq[:3] + face_eq[3]) <= 1e-9
    return inside
...
def _voxelize_convex_hull(
    ...
    """Voxelize a convex hull into a 3D grid, returning thickness map + volume."""
    ...
    for start in range(0, len(pts), batch_size):
        ...
        occupied[start:end] = _point_in_convex_hull(pts[start:end], hull_vertices)

    occupied = occupied.reshape((n_voxels, n_voxels, n_voxels))
    thickness_map_um = np.sum(occupied, axis=2, dtype=np.float64) * voxel_um
    volume_um3 = np.sum(occupied) * voxel_um**3
        ...
        for _ in range(max_attempts):
            ...
            if not overlap:
                ...
                if shape_type == "sphere":
                    ...
                    shape.initialize(rng, voxel_um=0.5)
                else:  # polyhedron
                    n_vertices = rng.integers(8, 20)
                    # Random aspect ratio for diverse habits
                    aspect = (
                        rng.uniform(0.4, 1.0),
                        rng.uniform(0.4, 1.0),
                        rng.uniform(0.3, 1.0),
...
def compute_complex_transmittance(
    ...
    """Compute complex transmittance t(x,y) for a single particle on the full N×N grid.

    t(x,y) = A(x,y) * exp(i * k * dn * h(x,y))

    The transmittance is only computed in a local bounding box around the particle
    and placed onto the full image for efficiency.
    ...
    # Determine bounding box in image pixels
    radius_um = _estimate_radius_um(shape)
    radius_px = radius_um / pixel_size_um + edge_sigma_px * 2 + 4  # add margin for blur
    ...
    # Local coordinate grids (in um, relative to particle center)
    x_local_px = np.arange(x1, x2, dtype=np.float64)
    y_local_px = np.arange(y1, y2, dtype=np.float64)
    xx_px, yy_px = np.meshgrid(x_local_px, y_local_px)

    x_um = (xx_px - x_pixel) * pixel_size_um
    y_um = (yy_px - y_pixel) * pixel_size_um

    # Get thickness map h(x,y) at local grid
    h_um = shape.get_thickness_map(x_um, y_um)

    # Binary mask from thickness
    mask = (h_um > 1e-9).astype(np.float64)

    # Smooth edge transition
    mask_smooth = gaussian_filter(mask, sigma=edge_sigma_px)
    ...
    if roughness > 0 and shape.shape_type != "sphere":
        ...
        # Smooth the roughness to avoid single-pixel artifacts
...
Examples:
  ...
  python generate_hologram_shape_classifier.py -o ./test -N 1024 --circular 15 --irregular 5 --snr 25
    ...
    p.add_argument("--edge-sigma", type=float, default=1.5, help="Edge blur sigma (pixels).")
